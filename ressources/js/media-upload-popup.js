...
jQuery( document ).ready( function( $ ) {
	...
	if ( 0 < ID ) {
		$( 'body' ).addClass( 'sti-modal' );

		var buttons = $( '.sti-modal .create-association' );
		...
		if ( 'image_id' in below.taxonomyImagesPlugin ) {
			buttons.each( function( i, e ) {
				var image_id = $( e ).parent().find( '.sti-button-image-id' ).val();
	...
	$( '.sti-modal .remove-association' ).live( 'click', function () {
		var button = $( this );
		...
		$.ajax( {
			...
			data: {
				...
				'wp_nonce' : $( this ).parent().find( '.sti-button-nonce-remove' ).val(),
				'tt_id'    : ID
				},
			cache: false,
	...
	$( '.sti-modal .create-association' ).live( 'click', function () {
		...
		$.ajax( {
			...
			dataType : 'json',
			data: {
				'action'        : 'sti_create_association',
				'wp_nonce'      : $( this ).parent().find( '.sti-button-nonce-create' ).val(),
				'attachment_id' : $( this ).parent().find( '.sti-button-image-id' ).val(),
				'tt_id'         : parseInt( ID )
				},
			success: function ( response ) {
				if ( 'good' === response.status ) {
					...
					$( '.sti-modal-control' ).each( function( i, e ) {
					...
					selector = parent.document.getElementById( 'sti-control-' + ID );

					/* Update the image on the screen below */
					$( selector ).find( '.sti-thumbnail img' ).each( function ( i, e ) {
